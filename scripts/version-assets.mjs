...
const imageFiles = (await readdir(path.join(root,'assets'),{recursive:true})).filter(name=>name.endsWith('.webp')).map(name=>`assets/${name}`).sort();
const normalize = text => text.replace(/\?v=[a-f\d]{12}/g,'').replace(/(<meta name="aether-release" content=")[^"]*("\s*\/?>)/,'$1RELEASE$2');
const contents = new Map();
const hash = createHash('sha256');
for (const name of [...textFiles,...imageFiles,'favicon.svg']) {
...
for (const [name,source] of contents) {
  let output = normalize(source).replace(/(['"`])([^'"`\s]+\.(?:js|css|webp|svg))\1/g,(match,quote,url) =>
